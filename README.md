...
- **Document Parsing**: For tasks like invoice processing, you can upload PDFs, create sections and fields (including table-like sections), and draw regions on pages to capture labels and values.
...
- [Tesseract OCR](https://github.com/tesseract-ocr/tesseract#installing-tesseract) (required for OCR features)
...
### Docker only
- `HOST_IP`: The host machine's local IP address. `launch.sh` detects this automatically. Set it manually if auto-detection fails (e.g., `export HOST_IP=192.168.1.100`).
...
- **Frontend cannot reach backend**:
    - **Docker**: Ensure `HOST_IP` is correctly set and that `NEXT_PUBLIC_API_BASE_URL` in `docker-compose.yaml` points to the correct backend address.
...
- **OCR not working (manual setup)**: Make sure Tesseract OCR is installed as a system package. See [installation instructions](https://github.com/tesseract-ocr/tesseract#installing-tesseract).
