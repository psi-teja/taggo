...
}> = ({ userData, dashboardLoading, settingsLoading, changePasswordLoading, onDashboardClick, onSettingsClick, onChangePasswordClick }) => {
...
const AccountDetails: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (accessToken) {
            try {
                ...
                const { username, email, is_superuser, groups, exp } = decoded;
                ...
                setUserData({ username, email, is_superuser, groups });
            } catch (error) {
    ...
    return (
        <div
            ...
            title={`Logged in as ${userData.username}`}
            aria-label={`Logged in as ${userData.username}`}
        >
            <div
...
const ChangePasswordIcon = () => (
    <svg className="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 11c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm0 0v2m0 4v2m-4-2h8" />
    </svg>
);
